...
import HealthyDiaryApp.model.Nutrition;
...
public class CreatedMenu {
    ...
    private MealComponent mealComponent = new MealComponent();
    ...
    private MainDishPairingComponent mainDishPairingComponent = new MainDishPairingComponent();
    ...
    private void selectRandomMenu(User user) {
        List<MealOption> breakfastOptions = mealOptionComponent.getAllMealOptionsByType(MealType.Breakfast);
        List<MealOption> lunchOptions = mealOptionComponent.getAllMealOptionsByType(MealType.Lunch);
        List<MealOption> dinnerOptions = mealOptionComponent.getAllMealOptionsByType(MealType.Dinner);
        List<MealOption> snackOptions = mealOptionComponent.getAllMealOptionsByType(MealType.Snack);
        ...
        if ((lunch.getAdditionalDishId() != null) && (dinner.getAdditionalDishId() != null)) {
            ...
            MealOption additionalDish = mealOptionComponent.getMealOptionById(lunch.getAdditionalDishId());
            MealOption dinnerAdditionalDish = mealOptionComponent.getMealOptionById(dinner.getAdditionalDishId());
    ...
    private void selectRandomMenuWithUserAllergy(User user) {
        ...
        List<Drink> drinkOptions = drinkComponent.getAllDrinks();
    ...
    private void printSelectedMenuAndCalculateCalories(User user,
        ...
        if ((dinnerAdditionalDish != null) && (dinner != null)) {
            ...
            breakfastId = mealComponent.findMealIdByName(breakfast.getName());
            snackId = mealComponent.findMealIdByName(snack.getName());
            snackSecondId = mealComponent.findMealIdByName(snackSecond.getName());
            lunchId = mealComponent.findMealIdByName(lunch.getName());
            additionalDishId = mainDishPairingComponent.findAdditionalDishIdByMainDish(lunch);
            dinnerId = mealComponent.findMealIdByName(dinner.getName());
            dinnerAdditionalDishId = mainDishPairingComponent.findAdditionalDishIdByMainDish(dinner);

        int breakfastDrinkId = breakfastDrink != null
                ? drinkComponent.findDrinkIdByName(breakfastDrink.getNameDrink())
                : null;

        int lunchDrinkId = lunchDrink != null
                ? drinkComponent.findDrinkIdByName(lunchDrink.getNameDrink())
                : null;

        int dinnerDrinkId = dinnerDrink != null
                ? drinkComponent.findDrinkIdByName(dinnerDrink.getNameDrink())
                : null;
      ...
      if (isNutritionWithinRange(totalNutrition, user)) {
          ...
          userSelectedMenuComponent.saveUserSelectedMenu(user.getPhoneNumber(),
    ...
    private MealOption generateRandomMeal(MealType mealType) {
        List<MealOption> options = mealOptionComponent.getAllMealOptionsByType(mealType);
    ...
    private MealOption getRandomOptionWithAllergy(List<MealOption> options, User user) {
        List<UserAllergy> userAllergies = userComponent.getUserAllergies(user);
    ...
    private boolean optionContainsAllergies(MealOption option, List<UserAllergy> allergies) {
        List<Ingredients> ingredients = ingredientComponent.getIngredientsByMealOption(option);
    ...
    private boolean drinkContainsAllergies(Drink drink, List<UserAllergy> allergies) {
        List<Ingredients> ingredients = ingredientComponent.getIngredientsByDrink(drink);
        for (Ingredients ingredient : ingredients) {
            for (UserAllergy allergy : allergies) {
                if (allergy.getIngredients().equals(ingredient)) {
                    return true;
    ...
    private Drink getRandomDrink() {
        List<Drink> drinks = drinkComponent.getAllDrinks();
        Random random = new Random();
