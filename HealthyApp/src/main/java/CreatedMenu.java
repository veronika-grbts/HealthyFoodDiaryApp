...
public class CreatedMenu {
    ...
    private void printSelectedMenuAndCalculateCalories(User user, MealOption breakfast, MealOption lunch, MealOption dinner, MealOption additionalDish, MealOption dinnerAdditionalDish, MealOption snack, MealOption snackSecond, Drink breakfastDrink, Drink lunchDrink, Drink dinnerDrink) {
        ...
        if (snack != null) {
            ...
            System.out.println("Перекус не будет " + breakfast);
        }
        if (snackSecond != null) {
            snackSecondId = hibernateMethods.findMealIdByName(snackSecond.getName());
            // Остальной код
        } else {
            System.out.println("Перекус два не будет " + breakfast);
        }
        if (lunch != null) {
        ...
        System.out.println("ПИТАТЕЛЬНАЯ  ЦЕННАСТЬ ДЛЯ ПОЛЬЗОВАТЕЛЯ "+totalNutrition);
        // Проверка соответствия общей питательной ценности нормам пользователя с учетом допустимого отклонения в 10%
      //  if (isNutritionWithinRange(totalNutrition, calorieCalculator.getNutritionNormForUser(user.getTotalCaloricUser()), 0.9)) {
      ...
      //  } else {
     //       MealOption newSnack = generateRandomMeal(MealType.Snack);
     //       printSelectedMenuAndCalculateCalories(user, breakfast, lunch, dinner, additionalDish, dinnerAdditionalDish, snack,newSnack, breakfastDrink, lunchDrink, dinnerDrink);
     //   }
    }
    ...
    private boolean isNutritionWithinRange(Nutrition nutrition, Nutrition norm, double deviation) {
        double proteinRange = norm.getProtein() * deviation;
        double fatRange = norm.getFat() * deviation;
        double carbohydratesRange = norm.getCarbohydrates() * deviation;
        return Math.abs(nutrition.getProtein() - norm.getProtein()) <= proteinRange &&
                Math.abs(nutrition.getFat() - norm.getFat()) <= fatRange &&
                Math.abs(nutrition.getCarbohydrates() - norm.getCarbohydrates()) <= carbohydratesRange;
    }
}
